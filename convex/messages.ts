...
export const get = query({args: {
    ...
    handler: async (ctx, args) =>{
        ...
        ("by_conversationId", (q)=> q.eq("conversationId", args.id)).order
        ("desc").collect()

        const messagesWithUsers = Promise.
        all(
            messages.map(async (message) => {
                const messageSender = await 
                ctx.db.get(message.senderId);

                if(!messageSender){
                    throw new ConvexError("Could not find sender of this message")
                }
